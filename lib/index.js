...
var AmazonScraper = /** @class */ (function () {
    function AmazonScraper(productUrl) {
        ...
        this.merchantNameSelector = '#sellerProfileTriggerId:first';
        ...
        this.categorySelector = '#SalesRank a';
        ...
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 2, , 3]);
                        return [4 /*yield*/, (0, request_promise_native_1.default)({
                                uri: object.url,
    ...
    AmazonScraper.prototype.processProductPage = function ($) {
        return __awaiter(this, void 0, void 0, function () {
            var title, price, merchantId, additionalInfo, merchantName, isAvailable, asin, productObject;
            return __generator(this, function (_a) {
                title = this.scrapeProductTitle($);
                price = this.scrapeProductPrice($);
                merchantId = this.scrapeMerchantId($);
                additionalInfo = this.scrapeAdditionalInfo($);
    ...
    AmazonScraper.prototype.processCheckoutPage = function ($, asin) {
        ...
        var hiddenInventoryConfirmation = Number($(inventoryText).val().split(',')[1]);
        ...
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    ...
                    case 5:
                        cartPage = _a.sent();
                        productObjectWithInventory = __assign(__assign({}, productObject), { inventorySize: this.processCartPage(cartPage, productObject.asin) });
    ...
    AmazonScraper.prototype.scrapeSalesRank = function ($) {
        ...
        var salesRankElemVal = $('#SalesRank')
            ...
            .replace('/[.]/gm', '');
        // const regex = /\+|-?(\d+.\d+.?).([^\(]+)/m;
        var regex = /[-]{0,1}[\d]*[\.]{0,1}[\d]+/m;
        var mainSalesRank = +salesRankElemVal.match(regex)[0].replace('.', '');
        var salesRankStr = $(salesRankItem).text().replace('#', '').replace('.', '').replace(',', '').replace('n°', '');
    ...
    AmazonScraper.prototype.scrapeAdditionalInfo = function ($) {
        var informations = {
            categories: [],
            ratings: [],
            createdOn: '',
        ...
        $('#productDetails_db_sections tr').each(function (i, elem) {
            var title = $(elem).find('.prodDetSectionEntry').text().trim().toLowerCase();
            var value;
            if (title === 'best sellers rank') {
                var rankData_1 = [];
                var digitRegex_1 = /#[1-9]\d*\b/gm;
                $(elem)
                    .find('td > span > span')
                    .each(function (i, elem) {
                    var rank = $(elem).text().trim().match(digitRegex_1)[0].replace('#', '');
                    var category = $(elem).find('a').text().toLowerCase().replace('see top 100 in', '').trim();
                    var categoryLink = $(elem).find('a').attr('href');
                    rankData_1.push({ categoryLink: categoryLink, category: category, rank: rank });
                    informations.categories.push(category);
                });
                informations.salesRank = rankData_1;
            }
                ...
                var totalRatings = $(elem).find('#acrCustomerReviewText').text().replace('ratings', '').trim();
    ...
    AmazonScraper.prototype.scrapeCategory = function ($) {
        var categoryMatch = $(this.categorySelector).attr('href').split('_');
        var category = categoryMatch[categoryMatch.length - 1].trim();
        return category;
    };
    /**
    ...
    AmazonScraper.prototype.scrapeProductAvailability = function ($) {
        var availability = $(this.isAvailableSelector).text().trim();
